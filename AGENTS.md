...
- 每次提交前都必须检查 `README.md` 是否需要同步更新。
- 只要提交涉及用户可见的功能、配置项、命令参数、使用方式、版本号、发布流程或运行行为，必须在同一个提交中更新 README，不能只修改代码后另行补文档。
- 纯测试、内部重构或不影响用户使用方式的改动，可以不修改 README；但提交说明中必须明确写出“已检查 README，本次无需更新”的结论。
- 提交前应结合真实差异复核 README 示例、版本号、命令和发布说明，避免保留过期内容。
